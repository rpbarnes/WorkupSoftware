...
def check_autosteps(threshold,values,figure_list = None,mask = None):
    ...
    ylabel('power (dBm)')
    return figure_list
def auto_steps(filename,threshold = -35, upper_threshold = 30.0, t_minlength = 0.5*60,minstdev = 0.1,showplots = True, showdebug = False,t_start=0,t_stop=60*1000,tolerance = 2,t_maxlen = inf,return_lastspike = False,title_name = 'power',first_figure = None):
    ...
    if showplots:
        figurelist = nextfigure(figurelist,'powerlog_raw'+ title_name)
        plot(t_ini/60,p_ini)
        ax = gca()
    ...
    while nextpos < len(t)-2:
        ...
        figurelist = nextfigure(figurelist,'powerlog' + title_name)
...
def auto_steps_array(power_nddata,threshold = -35, upper_threshold = 30.0, t_minlength = 0.5*60,minstdev = 0.1,showplots = True, showdebug = False,t_start=0,t_stop=60*1000,tolerance = 2,t_maxlen = inf,return_lastspike = False,first_figure = None,title_name = 'power',time_step = 0.5):
    r'Plot the raw power output in figure 1, and chop into different powers in plot 2'
    figurelist = figlistini_old(first_figure)
    p_ini = power_nddata.data
    t_ini = arange(0,len(power_nddata.data)*time_step,time_step) # jsut fix the time axis so the series starts at zero this makes things nicer for everybody
    #{{{ plot the raw power log, then just pull out the times we're interested in
    if showplots:
        figurelist = nextfigure(figurelist,'powerlog_raw_' + title_name)
        plot(t_ini/60,p_ini)
        ax = gca()
        ylim = list(ax.get_ylim())
        ylim[0] = -35
    ...
    while nextpos < len(t)-2:
        ...
        #flattened[nextpos-1] = curmean
    if showplots:
        figurelist = nextfigure(figurelist,'powerlogcut' + title_name)
        plot(t/60,spikes[0:len(t)],'r')
        plot(t_ini/60,p_ini.flatten(),'x-b')
        plot(t/60,flattened[0:len(t)]+2*flattenedstd[0:len(t)],'g',alpha=0.5)
        plot(t/60,track_curmean[0:len(t)],'y',alpha=0.5)
...
def process_t1(file,expno,usebaseline = None,showimage = None,plotcheckbaseline = None,saturation = False,first_figure = None,pdfstring = '',t1_offset_corr = None,verbose = False,showlinear = False,**kwargs):
    ...
    ### RYAN use the other uncommented version to make this work right
    #integral,figurelist = integrate(file,expno,first_figure = figurelist,pdfstring = pdfstring,phnum = [4],phchannel = [-1],**kwargs)
    t1name = r'$t_1$'
    ...
    #return integral,t1name,wait_time    

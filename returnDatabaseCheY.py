...
searchDict = {'macroMolecule':'CheY','setType':'kSigmaSeries','repeat':'0','temperature':'298','bindingPartner':'None'} 
...
if len(data.data):
    ax.errorbar(new.getaxis(xdim),new.data,yerr = new.get_error(),color = colorList[0],markersize = 10,fmt=styleList[0],label = '%s, %s'%(new.other_info.get('spinLabelSite'),new.other_info.get('repeat')))
else:
...
ax = fig.add_subplot(111) 
if len(data.data):
    ax.errorbar(old.getaxis(xdim)-3,old.data,yerr = old.get_error(),color = colorList[0],markersize = 10,fmt=styleList[0],label = '%s, %s'%(old.other_info.get('spinLabelSite'),old.other_info.get('repeat')))
else:
    print "empty data set for site %s and repeat %s"%(data.other_info.get('spinLabelSite'),data.other_info.get('repeat'))

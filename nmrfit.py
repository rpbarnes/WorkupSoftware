...
class t1power(fitdata):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.symbol_list = [k for j in range(0,multiplicity)
                for k in r'M_%s(0)'%alphaletter[j],r'T_1p%s'%alphaletter[j]]
        #self.starting_guesses = map(double,[array([1,0.1]*multiplicity)])
        self.starting_guesses = array([1,1,1]) #map(double,[array([1,1,1])])
...
class t1curve(fitdata):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.starting_guesses = map(double,[r_[-1.0,1.0,.8],r_[-20.0,20.0,.08],r_[-1.0,1.0,.08],r_[20,-20,2],r_[3,-3,0.1],r_[1,1,1],r_[0,0,1],r_[-100,100,0.03],r_[0.001,0.001,0.001],r_[1,-1,4.0]])

from pylab import *
import sys
sys.path += ['/opt/local/Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/tables/']
import textwrap
import tables
...
class nddata (object):
    ...
    def inttoval(self,thisaxis):
        # this will only work with two dimensions will need modified if we want to include temp and power
        xdata = self.getaxis(thisaxis)
        if len(self.dimlabels) > 1:
            for i in self.dimlabels:
                if i != thisaxis:
                    extradim = i
        # this is really not the way to do this...            
        int_list = []
        if len(self.dimlabels) >1:
            for j in range(len(self.getaxis(extradim))):
                int_list.append(simps(y = abs(self[extradim,j].data),x = self.getaxis(thisaxis)))
        else:
            int_list.append(simps(y = abs(self.data),x = self.getaxis(thisaxis)))
        self.data = array(int_list)
        return self
